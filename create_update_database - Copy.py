...
def search_key(key, dump_list, QUERY_URL):
    if (taxon[key] != False or taxon[key] != 'NA' or taxon[key] != ''):
        ...
        if key_list.count(',') > 0:
            ...
            for id in key_list:
                query_json = get_query_json(QUERY_URL + id)
                if isinstance(query_json, list):
                    for item in query_json:
                        # Empty query
                        if (query_json == None):
                            print(taxon_name + ' / ' + taxon[key] + ': no associated query.')
        ...
        # If 1 ID, do single query
            ...
            query_json = get_query_json(QUERY_URL + taxon[key])
            if isinstance(query_json, list):
                for item in query_json:
                    # Empty query
                    if (query_json == None):
...
for taxon in json_data:
    ...
    search_key('guid', accepted_matches, GUID_QUERY_URL)

    # # 2 --- Then, attempt with MycoBank Number
    search_key('mbNumber', accepted_matches, MBNUM_QUERY_URL)
